...
const getTimeRange = () => {
  const startTime = Math.floor(dayjs().subtract(1, 'day').startOf('day').valueOf() / 1000);
  ...
  await async.mapLimit(records.slice(1), 1, async (values: any[]) => {
    try {
      const record = values.reduce((acc, field, index) => {
        ...
        const value = ['参会人数', '累计入会设备数'].includes(fieldName) ? Number(field) : field;
        return { ...acc, [fieldName]: value };
      }, {});

      record['会议日期'] = dayjs(dayjs(record['会议开始时间']).format('YYYY-MM-DD')).valueOf();

      // 检查会议是否已存在
      const existingMeeting = meetingBitableData.find(
        (item) => item.fields['会议 ID'][0].text === record['会议 ID']
      );

      if (!existingMeeting) {
        console.log('上传新会议数据:', record['会议 ID']);
        const filteredRecord = Object.fromEntries(
          Object.entries(record).filter(([key]) => availableMeetingFields.includes(key))
        );
...
const syncMeetingData = async () => {
  ...
  try {
    ...
    await async.mapLimit(records.slice(1), 1, async (values: any[]) => {
      ...
      if (!participantList) {
        console.log('获取参会人数据失败', participantListTaskId);
        process.exit(1);
      }

      await feishuAPI.downloadFile(participantList, `participantList-${meetingNo}`);
